...
type SchemaItemProps = {
  ...
  rootName: string;
  rootNameEditable?: boolean;
  rootTypeDisable?: boolean;
  handleRootNameChange?: (name: string) => void;
};
...
function SchemaItem(props: SchemaItemProps) {
  ...
  useEffect(() => {
    ...
    setSchemaTitle(props.schema.title);
  }, [props.schema]);
      ...
      <Row align={"middle"} style={{ paddingBottom: 10 }}>
        ...
        <Col flex={"auto"} style={{ marginLeft: 5 }}>
          <Input
            ...
            disabled={(isRoot && !rootNameEditable) || isArrayItems}
            value={isRoot ? rootName : propertyName}
            placeholder={"属性名称"}
            onBlur={() => {
            ...
            onChange={(name) => {
              ...
              if (isRoot && handleRootNameChange) {
                handleRootNameChange(value);
              } else {
        ...
        <Col flex={"95px"} style={{ marginLeft: 5 }}>
          <Select
            ...
            disabled={isRoot && rootTypeDisable}
            onChange={(type) => {
              if (changeSchema) {
