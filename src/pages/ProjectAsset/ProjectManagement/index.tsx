...
} from "@/services/flode/projectGroupController";
import { listRepository } from "@/services/flode/repositoryController";
import {
  getTemplateDetail,
  listTemplate,
...
type ProjectModalProp<T> = {
  ...
  onFinish: ((formData: T) => void | Promise<boolean | void>) &
        ...
        onValuesChange={(changedValues, values) => {
          ...
          if (changedValues.templateId) {
            getTemplateDetail({ id: changedValues.templateId }).then((resp) => {
              const automations: API.ProjectAutomationDTO[] =
                form.getFieldValue("automations");
                      ...
                      <Col flex={"auto"}>
                        <Row gutter={4}>
                          ...
                          <ProFormDependency name={["automations"]}>
                            {({ automations }) => {
                              const triggerAction =
                                automations[name]?.triggerAction;
                              return (
                                <>
                                  <Col span={6}>
                                    {showRequirementType(triggerAction) && (
                                      <ProFormSelect
                                        ...
                                        options={
                                          selectedTemplate?.requirementTypes
                                        }
                                        fieldProps={{
                                          fieldNames: IdName,
                                          mode: "multiple",
                              ...
                              <ProFormDependency name={["automations"]}>
                                {({ automations }) => {
                                  const automationAction =
                                    automations[name]?.automationAction;
                                  return (
                                    <>
                                      <Col
                                        span={
